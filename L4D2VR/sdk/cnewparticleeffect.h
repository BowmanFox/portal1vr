...
public:
	inline void SetControlPoint(int nWhichPoint, const Vector& v) {
		...
		if (!g_Game || !g_Game->m_Offsets)
			return;

		static tSetControlPoint oSetControlPoint = nullptr;
		if (!oSetControlPoint)
			oSetControlPoint = reinterpret_cast<tSetControlPoint>(g_Game->m_Offsets->SetControlPoint.address);
		if (!oSetControlPoint)
			return;
	...
	inline void StopEmission(bool bInfiniteOnly = false, bool bRemoveAllParticles = false, bool bWakeOnStop = false, bool bPlayEndCap = false) {
		...
		if (!oStopEmission)
			oStopEmission = reinterpret_cast<tStopEmission>(g_Game->m_Offsets->StopEmission.address);
		if (!oStopEmission)
			return;
