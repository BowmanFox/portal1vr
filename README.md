...
The tester also confirmed controller-directed portal placement and independent arm movement in an early room without the gun. The custom VPK supplies the authored hand textures and rebuilt mirrored hand models. Active SteamVR skeletal input uses the full open-to-closed curl range. Model symmetry, normalized weights, finger motion, and the Release x86 build pass local checks. The September 14 Pico test loaded the textured models after rebuilding the archive with Portal's own VPK packer; subsequent hand-flip and body-obstruction feedback produced another test build. Controller pickup behavior, the latest hand orientation/body visibility, portal transitions, pause-menu behavior, and a complete campaign playthrough still require user validation.
...
`FirstPersonBody=true` enables the local body in the VR eye views and the separate desktop mirror (`RenderWindow=1`). Keep `FirstPersonBodyHideUpper=true` for the non-obstructive view; it removes the near-camera head and arm branches while retaining the torso and legs, without changing the source model. Set `FirstPersonBody=false` to disable the feature, or set `FirstPersonBodyHideUpper=false` only when testing the unmasked full player model.
